...
namespace motionplanner
{
  ...
  **/
  ...
  void Pf3Tracker::SetAlgoParam()
  {
    ...
    // 测试使用，从文件中读取控制参数
    ifstream infile("pf3_param.txt");
    string line, token;
    getline(infile, line);
    
    stringstream ssline(line);
    getline(ssline, token, ' ');
    lqr_param_.w_x = atof(token.c_str());
    getline(ssline, token, ' ');
    lqr_param_.w_y = atof(token.c_str());
    getline(ssline, token, ' ');
    lqr_param_.w_yaw = atof(token.c_str());
    getline(ssline, token, ' ');
    lqr_param_.w_vyaw = atof(token.c_str());
                                   ...
                                   size_t *n_idx, bool backward)
  {
    ...
    if(m_begin == 0) m_begin++;
      ...
      if (dis > 0.5 || fabs(theta) > 15.0 / 180.0 * 3.14)
      {
    ...
    if (state_ == RobotState::move)
    {
      ...
      // ROBOT at stop point : solve1: 
      double tt = fabs(DiffRad(atan2(traj.at(end_i - 1).y - robot.y,
                             traj.at(end_i - 1).x - robot.x), robot.theta));
      double dis_temp = getDistance(robot, traj.at(end_i - 1));
      if (dis_temp < 0.1 || ((min_id + 5) > end_i && ((!backward && tt > M_PI_2) ||(backward && tt < M_PI_2))))
      {
        Init();
        *n_idx = end_i - 1;
        ...
        if (agv_type_ == AGVTYPE::AGVDIFF_S200)
        {
          v_curv = v_curv < 0.1 ? 0.1 : v_curv;
        }
        ...
        *n_idx = min_id;
        ...
        vec_cmd.push_back(now_cmd);
        if(vec_cmd.size() > 10) {
          vec_cmd.erase(vec_cmd.begin());
        }
