...
int main(int argc, char **argv) {
  ...
  // 模拟的车，会自己计算坐标
  std::unordered_map<int, std::shared_ptr<AGV_Model>> agv_models = {
      {10, std::make_shared<AGV_Model>(10, 0, 2, 0)},
      {6, std::make_shared<AGV_Model>(6, 0, -2, 0)}};
  // 虚拟的车
  Pose vl(0, 0, 0);

  Global::set_pose(10, Pose(0, 2, 0));
  Global::set_pose(6, Pose(0, -2, 0));
  ...
  while (true) {
    // 计算虚拟大车的位置
    // Pose vl = task_handler.CalcVirtualCenter();
    // LOG(INFO) << vl;
    // 计算速度
    auto state = tracker.Track(traj, 0.5, traj_s, 0, traj.size(), vl, v, w, &v,
                               &w, &index);
    ...
    for (const auto &it : agvs_res) {
      ...
      agv_models[it.first]->setTarget(it.second);
      Global::set_agv_job_state(it.first, JOBSTATE::following);
    }

    std::this_thread::sleep_for(std::chrono::milliseconds(50));
    for (const auto &it : agvs_res) {
      Global::set_pose(it.first, agv_models[it.first]->getPose()); // 跟速度有关
      // Global::set_pose(it.first, it.second);  // 一个一个点遍历的，跟速度无关
    }
  }

  for(auto agv : agv_models){
    agv.second->Stop();
  }
